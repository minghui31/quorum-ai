...
一次裁定告诉你一个答案；**议事会集合（ensemble）**告诉你这个答案有多大争议：

```bash
quorum simulate examples/serious_case.zh.yaml --runs 20
```

蒙特卡洛审议并行召开 N 个相互独立的议事会（`--runs 20` = 120 个智能体实例），
输出的是*分布*：众数结局、翻转率、以及在多次审议中反复出现的异议。
对真正的决定而言，**分歧的形状本身就是信息**。成本受 `QUORUM_MAX_RUNS`
护栏保护（默认上限 50）；mock 模式下完全免费。
